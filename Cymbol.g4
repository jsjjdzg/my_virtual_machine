...
funDecl
	:  {seqNum = 0; tempNum = 0; paramRenameMap = new HashMap<String, String>();}
	    (FUNCTION fn=ID
	   {$funDecl::funcName=fn.getText(); List<Instruction> insList = new ArrayList<Instruction>(); functionMap.put(fn.getText(), insList); this.insList=insList;}
	   parmeters block)
;
...
paramDecl
	:	 (PARAM p=ID)	{String temp="t"+ tempNum++; paramRenameMap.put(p.getText(),temp);}
;
...
:
    {Instruction instruct1 = null, instruct2 = null;insList.add(new Instruction(seqNum++, "rem",  "beginIf"));}
	   (IF expr {instruct1 =new Instruction(seqNum++, "jz"); insList.add(instruct1);}
	    ...
	    s2=(stmt|NOP)){int seq2=seqNum;instruct2.oprand1=seq2+"";insList.add(new Instruction(seqNum++, "rem", "endIf"));}
	;
...
:
      {level +=1;}
       (ASSIGN v=ID expr) {if(level > 1){addInstruction(new Instruction(seqNum++, "dup"));} String var =paramRenameMap.get(v.getText()); if(null==var){String temp="t"+ tempNum++; paramRenameMap.put(v.getText(),temp);var=temp;} addInstruction(new Instruction(seqNum++, "asn", var));} 	{level -=1;}
